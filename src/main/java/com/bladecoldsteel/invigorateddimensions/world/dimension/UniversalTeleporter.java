...
public class UniversalTeleporter implements ITeleporter {
        ...
        for (int k1 = -1; k1 < 3; ++k1) {
            for (int i2 = -1; i2 < 4; ++i2) {
                if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
                    ...
                    this.world.setBlock(mutablePos, frameBlock.defaultBlockState(), 3);
                }
            }
        }

        BlockState portalState = portalBlock.defaultBlockState().setValue(GeneralizedPortalBlock.AXIS, axis);

        for (int j2 = 0; j2 < 2; ++j2) {
            for (int l2 = 0; l2 < 3; ++l2) {
                mutablePos.setWithOffset(blockpos, j2 * direction.getStepX(), l2, j2 * direction.getStepZ());
                this.world.setBlock(mutablePos, portalState, 18);
            }
        }
    ...
    protected Optional<TeleportationRepositioner.Result> getOrMakePortal(Entity entity, BlockPos pos) {
        ...
        if (existingPortal.isPresent()) {
            ...
            Direction.Axis portalAxis = this.world.getBlockState(entity.portalEntrancePos).getOptionalValue(GeneralizedPortalBlock.AXIS).orElse(Direction.Axis.X);
