...
public class WaterPortalStructure extends AbstractSurfaceStructure {

    public WaterPortalStructure(Codec<NoFeatureConfig> codec) {
        super(codec);
    }

    @Override
    public IStartFactory<NoFeatureConfig> getStartFactory() {
        return WaterPortalStructure.Start::new;
    }
    ...
    public static class Start extends StructureStart<NoFeatureConfig> {
        ...
        public void generatePieces(DynamicRegistries dynamicRegistries, ChunkGenerator chunkGenerator, TemplateManager templateManager, int chunkX, int chunkZ, Biome biome, NoFeatureConfig featureConfig) {
            ...
            java.util.function.BiFunction<Integer, Integer, Boolean> isLand =
                    (x,z_) -> {
                        int ySurf = chunkGenerator.getBaseHeight(x, z_, Heightmap.Type.WORLD_SURFACE_WG);
                        if (ySurf <= 0) return false;
                        BlockState top = chunkGenerator.getBaseColumn(x, z_).getBlockState(BlockPos.of(Math.max(0, ySurf - 1)));
                        return !top.getMaterial().isLiquid() && top.getMaterial().isSolid();
                    };
            ...
            if (!(isLand.apply(x1, z1) && isLand.apply(x2, z2) && isLand.apply(x3, z3) && isLand.apply(x4, z4))) {
                return;
            }
            ...
            JigsawManager.addPieces(
                    dynamicRegistries,
                    new VillageConfig(() -> dynamicRegistries.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY).get(new ResourceLocation(InvigoratedDimensions.MOD_ID, "water_portal/start_pool")), 10),
