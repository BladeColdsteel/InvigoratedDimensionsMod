...
public class DimensionTeleportCommand {
    public static void register(CommandDispatcher<CommandSource> dispatcher) {
        LiteralArgumentBuilder<CommandSource> dtc = Commands.literal("tpdim")
                .requires(commandSource -> commandSource.hasPermission(2));
        dtc.then(Commands.argument("targets", EntityArgument.entities())
    ...
    private static int teleportToPosWithDim(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ServerWorld destWorld = DimensionArgument.getDimension(context, "dimension");
        return teleportToPos(context, target -> destWorld);
    }
